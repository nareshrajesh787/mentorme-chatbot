...
const OFFICIAL_HOME_SOURCE: ChatSource = {
  id: "mentorme-official-home",
  title: "MentorMe North Georgia — Official Website",
  url: `${ORG.canonicalOrigin}/`,
...
function automaticGreetingAnswer(
  ...
  const responses: Record<AutomaticGreetingLanguage, string> = {
    es: "¡Hola! Puedo ayudarte a encontrar información aprobada de MentorMe. ¿En qué puedo ayudarte?",
    fr: "Bonjour ! Je peux vous aider à trouver des informations approuvées sur MentorMe. Que souhaitez-vous savoir ?",
    "hi-latn": "Namaste! Main MentorMe ke baare mein approved jaankari dhoondhne mein aapki madad kar sakta hoon. Aap kya jaanna chahenge?",
    "ar-latn": "Ahlan! Mumkin asaedak fi al-hosool ala maaloomat muetamada an MentorMe. Sho habeb taaraf?",
    pt: "Olá! Posso ajudar você a encontrar informações aprovadas sobre MentorMe. O que você gostaria de saber?",
    it: "Ciao! Posso aiutarti a trovare informazioni approvate su MentorMe. Cosa vorresti sapere?",
    "vi-latn": "Xin chao! Toi co the giup ban tim thong tin da duoc phe duyet ve MentorMe. Ban muon biet gi?",
    tl: "Kumusta! Matutulungan kitang makahanap ng aprubadong impormasyon tungkol sa MentorMe. Ano ang gusto mong malaman?",
    "zh-latn": "Ni hao! Wo keyi bang ni chazhao MentorMe de yanzheng xinxi. Ni xiang liaojie shenme?",
  };
...
export function getLocalConversationalResponse(
  ...
  if (
    ...
    return answered(
      language === "es"
        ? "¡Hola! Puedo ayudarte a encontrar información confirmada sobre los programas de mentoría, voluntariado, inscripción, ubicaciones, contactos y eventos de MentorMe. ¿En qué puedo ayudarte?"
        : language === "en" || !detectedGreeting
          ? "Hi! I can help you find confirmed information about MentorMe’s mentoring programs, volunteering, enrollment, locations, contacts, and events. What would you like help with?"
  ...
  if (
    ...
    return answered(
      language === "es"
        ? "Puedes preguntar sobre programas de mentoría, cómo convertirte en mentor voluntario, cómo inscribir a un niño, requisitos y verificación de antecedentes, ubicaciones, contactos y próximos eventos. Pregunta con naturalidad y haz preguntas de seguimiento si necesitas más detalles. No puedo consultar solicitudes ni emparejamientos personales; cuando la información aprobada no confirme una respuesta, te dirigiré al personal."
        : "You can ask about mentoring programs, how to become a volunteer mentor, how to enroll a child, background-check requirements, locations, contacts, and upcoming events. Ask naturally and use follow-up questions if you need more detail. I can’t check a personal application or mentor match, and I’ll direct you to staff when the approved information doesn’t confirm an answer.",
    );
  }
  ...
  if (/^(thanks|thank you|thank you so much|thanks so much|thx)$/.test(normalized)) {
    return answered(
      language === "es"
        ? "¡Con gusto! Avísame si tienes otra pregunta sobre MentorMe."
        : "You’re welcome! Let me know if you have another question about MentorMe.",
    );
  }
