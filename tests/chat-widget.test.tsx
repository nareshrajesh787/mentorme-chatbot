...
describe("chat widget lifecycle", () => {
  it("preserves the browser-session draft when minimized", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Open MentorMe assistant" }));

    expect(
      (screen.getByLabelText(
        "Ask the MentorMe information assistant",
  ...
  it("starts a fresh conversation after the user closes the chat", () => {
    render(<ChatWidget initialOpen />);
    fireEvent.change(
      screen.getByLabelText("Ask the MentorMe information assistant"),
    ...
    expect(screen.getAllByText(/approved information from MentorMe/i)).toHaveLength(1);
  });
  ...
  it("shows a delayed, non-focusing suggestion that opens the chat", () => {
    ...
    expect(screen.getByText("Ask the MentorMe chatbot")).toBeDefined();
    expect(document.activeElement).toBe(document.body);

    fireEvent.click(
      screen.getByRole("button", {
        name: "Need help? Open MentorMe chatbot",
      }),
    );
