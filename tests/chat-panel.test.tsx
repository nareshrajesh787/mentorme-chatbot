...
describe("ChatPanel request pipeline", () => {
  it("uses approved-information wording without exposing staff-source labels", () => {
    ...
    expect(screen.getByText(/approved information from MentorMe/i)).toBeDefined();
  ...
  it("sends a quick action through the normal JSON request pipeline", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockHttpResponse(answered("You can become a mentor through MentorMe.")),
    );
    ...
    expect(JSON.parse(init.body as string)).toEqual({
      message: "How do I become a mentor?",
      history: [],
      language: "auto",
    });
  });
  ...
  it("sends typed input and follow-up history with only role and content", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        mockHttpResponse(answered("MentorMe offers confirmed food help.")),
      )
      .mockResolvedValueOnce(
    ...
    await screen.findByText("MentorMe offers confirmed food help.");
    ...
    expect(JSON.parse(secondInit.body as string)).toEqual({
      ...
      history: [
        ...
        {
          role: "assistant",
          content: "MentorMe offers confirmed food help.",
        },
      ],
      language: "auto",
    });
  ...
  it("shows a visible language selector and sends localized Spanish questions", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockHttpResponse(answered("Puedes convertirte en mentor en MentorMe.")),
    );
    vi.stubGlobal("fetch", fetchMock);
    render(<ChatPanel onMinimize={vi.fn()} onClose={vi.fn()} />);
    ...
    expect(JSON.parse(init.body as string)).toEqual({
      message: "¿Cómo puedo convertirme en mentor?",
      history: [],
      language: "es",
    });
  });
